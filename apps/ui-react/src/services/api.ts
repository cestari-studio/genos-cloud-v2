...
async function edgeFn<T = unknown>(fnName: string, body?: unknown): Promise<T> {
  ...
  if (!res.ok) {
    ...
    // Global 401 Interceptor with retry/refresh logic
    if (res.status === 401) {
      console.warn('genOS: 401 Unauthorized from Edge Function. Attempting session refresh...');

      const { data: { session: refreshedSession }, error: refreshError } = await supabase.auth.refreshSession();

      if (!refreshError && refreshedSession) {
        console.info('genOS: Session refreshed successfully. Retrying request...');
      ...
      console.error('genOS: Session refresh failed or retry still unauthorized. Force-clearing session.');
      const msg = 'Sessão expirada. Por favor, faça login novamente para continuar.';
      localStorage.removeItem('genOS_activeUserEmail');
      localStorage.removeItem('genOS_activeClient');
      // Dispatch event so UI can react (e.g. show login modal or redirect)
...
export const api = {
  ...
  async loadChildTenants(parentId: string): Promise<Tenant[]> {
    try {
      const { data, error } = await supabase
        .from('tenants')
        .select('*')
      ...
      console.warn('genOS: child tenant fetch failed', err);
      return [];
    }
  },
  ...
  getMe: async (forceRefresh?: boolean): Promise<MeResponse> => {
    ...
    try {
      ...
      // 2.5 Ensure Agency Master visibility if role is agency_operator or super_admin
      const isAgencyLevel = role === 'agency_operator' || role === 'super_admin';
  ...
  async updateTenantConfig(tenantId: string, updates: Partial<MeResponse['config']>): Promise<void> {
  ...
  async signContract(tenantId: string, metadata: any = {}): Promise<void> {
    ...
    await this.updateTenantConfig(tenantId, { contract_signed: true } as any);
  },

  async saveBrandDNA(tenantId: string, dna: any): Promise<void> {
    const { error } = await supabase
      .from('brand_dna')
      .upsert({
        tenant_id: tenantId,
  ...
  async getBrandDNA(tenantId: string): Promise<any | null> {
    const { data, error } = await supabase
      .from('brand_dna')
      .select('*')
      .eq('tenant_id', tenantId)
  ...
  async generateDNAProposal(tenantId: string, briefing: any): Promise<any> {
    return await this.edgeFn('content-factory-ai', {
  ...
  async vectorizeDNA(tenantId: string, finalDna: any): Promise<void> {
    await this.edgeFn('content-factory-ai', {
      action: 'vectorize_dna',
      tenantId,
      dna: finalDna
    });
  },
  ...
  async completeOnboarding(tenantId: string): Promise<void> {
    await this.updateTenantConfig(tenantId, { onboarding_completed: true } as any);
  },

  async getGeoIntelligence(tenantId: string): Promise<any> {
    return this.edgeFn('content-factory-ai', { action: 'get-geo-intel', tenantId });
  },

  async callCopilot(tenantId: string, message: string): Promise<any> {
    return this.edgeFn('content-factory-ai', { action: 'chat', tenantId, message });
  },

  async getQuantumPulse(tenantId: string, content: string): Promise<any> {
    return this.edgeFn('content-factory-ai', { action: 'get-qhe-score', tenantId, content });
  },

  async bulkExport(tenantId: string, postIds: string[]): Promise<Blob> {
    return this.edgeFnBlob('content-factory-ai', { action: 'export_zip', postId: postIds, tenantId });
  },

  // ─── GEO INTELLIGENCE (Módulo 7) ──────────────────────────────────────────
  async getGeoAnalytics(tenantId: string): Promise<any[]> {
    const { data, error } = await supabase
  ...
  async calculateQuantumResonance(tenantId: string, analyticsId: string, semanticClusters: any[]): Promise<any> {
    return this.edgeFn('geo-intelligence-processor', {
      action: 'calculate-quantum-resonance',
